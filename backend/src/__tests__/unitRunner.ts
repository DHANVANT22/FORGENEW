...
import { PlanValidator } from '../utils/PlanValidator';
...
async function runUnitTests() {
  ...
  const unestimatedRes = TierEngine.score({ users: null, data: null, compliance: null }, cutoffs);
  ...
  // Test Simple Tier (< 8)
  const simpleRes = TierEngine.score({ users: 1, compliance: 1, urgency: 1 }, cutoffs);
  assert(simpleRes.tier === 'Simple', `TierEngine score for simple profile is Simple (Got: ${simpleRes.tier})`);

  // Test Standard Tier
  const standardRes = TierEngine.score({ users: 2, data: 2, compliance: 1, urgency: 2 }, cutoffs);
  ...
  const enterpriseRes = TierEngine.score({ users: 3, data: 4, compliance: 4, scale: 4, urgency: 3, integrations: ['A', 'B', 'C'] }, cutoffs);
  ...
  const riskRes1 = RiskService.scoreHypothetical({ timelinePressure: 'High', integrations: 3 }, baseRisk);
  ...
  // Capped at 100
  const riskResCapped = RiskService.scoreHypothetical({ timelinePressure: 'High', integrations: 20, realTime: true, compliance: true, roles: 10 }, baseRisk);
  ...
  const emptyPlan = PlanValidator.validate(null);
  assert(emptyPlan.valid === false && emptyPlan.error === 'Plan is empty', 'PlanValidator flags empty input');

  const invalidPlan = PlanValidator.validate({ wrongField: 123 });
  assert(invalidPlan.valid === false, 'PlanValidator flags invalid root structure');

  const validPlan = PlanValidator.validate([{ Plan: { 'Node Type': 'Seq Scan', 'Relation Name': 'User' } }]);
