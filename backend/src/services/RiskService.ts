...
export class RiskService {
   ...
   */
  static scoreHypothetical(inputs: RiskInputs, baseRisk: { schedule: number, budget: number, communication: number, scopeDrift: number } = { schedule: 10, budget: 10, communication: 10, scopeDrift: 10 }) {
    ...
    if (inputs.timelinePressure === 'High') { schedule += 40; budget += 20; }
    else if (inputs.timelinePressure === 'Medium') { schedule += 20; budget += 10; }

    // integrations -> budget & scopeDrift
    if (inputs.integrations) { budget += inputs.integrations * 8; scopeDrift += inputs.integrations * 5; }

    // realTime -> schedule & budget
    if (inputs.realTime) { schedule += 20; budget += 25; }

    // compliance -> budget & scopeDrift
    if (inputs.compliance) { budget += 20; scopeDrift += 15; }

    // roles -> communication & scopeDrift
    if (inputs.roles) { communication += inputs.roles * 10; scopeDrift += inputs.roles * 4; }
  ...
  static async computeAll() {
    ...
    for (const project of projects) {
      ...
      let lastStaffReply: Date | null = null;
      for (const conv of project.conversations) {
        for (const msg of conv.Message) {
          if (msg.User?.role === 'DEV' || msg.User?.role === 'SUPER_ADMIN' || msg.User?.role === 'MANAGER') {
            if (!lastStaffReply) lastStaffReply = msg.createdAt;
          } else {
            if (!lastClientMessage) lastClientMessage = msg.createdAt;
          }
        }
      }
      ...
      if (lastClientMessage && lastStaffReply && lastClientMessage > lastStaffReply) {
        ...
        if (replyLatency > 48) communicationRisk += 40;
        else if (replyLatency > 24) communicationRisk += 20;
      ...
      if (project.ProjectRiskSnapshot[0]) {
        previousScores = project.ProjectRiskSnapshot[0].axisScores as any;
      }
      ...
      const snapshot = await prisma.projectRiskSnapshot.create({
        data: {
          id: uuidv4(),
      ...
      if (criticalAxes.length > 0) {
        ...
        const snoozedAlerts = await prisma.riskAlertSnooze.findMany({
