...
export class ClientProjectController {
  ...
  public static async submitProjectBrief(req: ClientAuthRequest, res: Response) {
    try {
      const clientId = req.client?.id;
      let projectId = req.client?.projectId;
      ...
      if (!projectId) {
        const clientAcc = await prisma.clientAccount.findUnique({ where: { id: clientId } });
        projectId = clientAcc?.projectId || undefined;
      }

      if (!projectId) {
        const defaultProj = await prisma.project.findFirst({ orderBy: { createdAt: 'desc' } });
        if (defaultProj) {
          projectId = defaultProj.id;
          await prisma.clientAccount.update({ where: { id: clientId }, data: { projectId: defaultProj.id } }).catch(console.error);
        }
      }
      ...
      const clientAccount = await prisma.clientAccount.findUnique({ where: { id: clientId } });
      ...
      io.to(`project_${projectId}`).emit('receive_message', formattedMessage);
